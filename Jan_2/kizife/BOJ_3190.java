...
public class BOJ_3190 {
    ...
    public static int playGame(int n, Map<Integer, Character> dirChange) {
        ...
        while (true) {
            ...
            //머리 이동
            int[] head = snake.peekFirst(); //deque의 첫번째요소(머리) 반환함. deque상태 변하지X
            ...
            if (map[nextX][nextY] == 1) {
                ...
                //사과가 없다면
                snake.addFirst(new int[]{nextX, nextY}); //맨앞(즉. 머리위치)에 요소 추가
                map[nextX][nextY] = 2;

                int[] tail = snake.pollLast(); //마지막위치(꼬리) 제거하고 반환.. deque 줄어듦
                map[tail[0]][tail[1]] = 0; //꼬리위치는 0 삽입
            }

            //방향 전환
            if (dirChange.containsKey(time)) { //time key가 dirChange에 존재한다면
